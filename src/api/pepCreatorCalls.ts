...
export const fetchPoliticalProfile = async (politician: string, customSource?: string): Promise<TPoliticalProfileResponse | null> => {
    ...
    const response = await fetch("/.netlify/functions/pepCreator", {
        ...
        body: JSON.stringify({ politician: cleanedPolitician, customSource: cleanedSource })
    });
